...
exports.createGeneration = (blockHeight, blockReward, feeReward, recipients, poolAddress, poolHex, coin, masternodeReward, masternodePayee, masternodePayment) => {
    ...
    // Pay Treasury from raw RPC data, if requested
    if (coin.rpcTreasury && rpcData.treasury !== undefined || rpcData.treasury !== null) 
    { 
        blockReward -= rpcData.treasury.amount; 
        tx.addOutput(new Buffer(rpcData.treasury.hex, 'hex')); 
    }
    
    // Newest Masternode format, based on DASH 0.12 
    if (coin.rpcMasternodeDashFormat && rpcData.masternode !== undefined || rpcData.treasury !== null) 
    { 
        var masternodeamount = rpcData.masternode.amount; 
        blockReward -= masternodeamount; 
        tx.addOutput(scriptCompile(bitcoin.address.fromBase58Check(rpcData.masternode.payee).hash), masternodeamount); 
    }
