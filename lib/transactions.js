...
exports.createGeneration = (rpcData, blockReward, feeReward, recipients, poolAddress, poolHex, coin, masternodeReward, masternodePayee, masternodePayment) => {
    ...
    let blockHeightSerial = (rpcData.blockHeight.toString(16).length % 2 === 0 ? '' : '0') + rpcData.blockHeight.toString(16)

    let height = Math.ceil((rpcData.blockHeight << 1).toString(2).length / 8)
    ...
    if (masternodePayment === false || masternodePayment === undefined) {
        ...
        if (coin.payFoundersReward === true && ((coin.maxFoundersRewardBlockHeight >= blockHeight || coin.treasuryRewardStartBlockHeight || coin.treasuryRewardUpdateStartBlockHeight) || coin.payAllFounders === true)) {
