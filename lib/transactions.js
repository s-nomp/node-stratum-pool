...
exports.createGeneration = (rpcData, blockReward, feeReward, recipients, poolAddress, poolHex, coin, masternodeReward, masternodePayee, masternodePayment) => {
    ...
    if ((masternodePayment === false || masternodePayment === undefined) && !rpcData.coinbase_required_outputs) {
     ...
     // ANON getblocktemplate provides an array of objects (coinbase_required_outputs) 
     // it contains all the required coinbase outputs (masternode, developement fund, superblocks)
     // each object has the following keys: amount, script (hex), type.
     // 'type' provides short info about the purpose of the output
     // for example 'masternode', 'development' or 'superblock'
    */
     ...
     let required_outputs_total = 0; 
        ...
        // no founders rewards :)
            ...
            /*
            Note: For ANON coin, it enters this code when fullnode doesn't return any masternode payee
            This should never happen on mainnet, since there are plenty of masternodes. But, it is possible on ANON testnet, since there are not so many masternodes.
            */
