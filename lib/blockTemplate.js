...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    this.calculateTrees = function() {
        daemon.cmd(
            'getSidechainMerkleRoots',
            [this.rpcData.transactions.map(el => el.data),this.rpcData.certificates],
            result => result.error ? callback(result.error) : this.getTrees(result[0].response)
        )
    }

    this.getTrees = function(response){
        this.merkleRoot = response.merkleTree;
        this.hashReserved = response.scTxsCommitment;
        this.merkleRootReversed = util.reverseBuffer(new Buffer(this.merkleRoot, 'hex')).toString('hex');
    }

    if (this.rpcData.version == 3) {
